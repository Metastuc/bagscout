...
function SideNavLink({ label, value }: { label: string; value: NavigationTab }) {
    const { activeTab, setActiveTab, setTablePageIndex } = useClientViewState(
        useShallow((state) => ({ activeTab: state.activeTab, setActiveTab: state.setActiveTab, setTablePageIndex: state.setTablePageIndex })),
    );
    ...
    return (
        <li
            className={cn(
                ...
                "relative",
                isActive ? "bg-primary/15 text-primary font-medium" : "text-primary hover:bg-primary/10",
            )}
            onClick={() => {
                setActiveTab(value);
                setTablePageIndex(0);
        ...
        >
            {isActive ? <span className="bg-primary absolute top-0 left-0 h-full w-1 rounded-r" /> : null}
            {label}
        </li>
    );
}
