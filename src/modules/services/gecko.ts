...
export function getGeckoPools(poolIds: string[], deps: AppDependencies): Promise<Array<GeckoPoolData>> {
    const idsParam = poolIds.join(",");

    return fetch(`https://api.geckoterminal.com/api/v2/networks/solana/pools/multi/${idsParam}`)
        .then(async (response) => await response.json())
