          ...
          <section className="my-4 space-y-2">
            {noPoolData || notIndexed ? (
              <div className="border-muted flex h-115 items-center justify-center rounded-md border bg-black">
                <span className="text-gray-400">
                  {noPoolData
                    ? "No pool address — chart unavailable"
                    : "Not yet indexed on GeckoTerminal"}
                </span>
              </div>
            ) : (
              <iframe
            ...
            {chartPoolAddress ? (
              <div className="text-xs text-gray-500">
                <a
                  href={`https://www.geckoterminal.com/solana/pools/${chartPoolAddress}`}
                  target="_blank"
                  rel="noreferrer"
                >
            ...
            {[
              {
                ...
                value: displaySafeValue(
                  safeNumber(geckoData?.attributes.base_token_price_usd),
                  formatPriceToUSD,
                ),
              },
              {
                label: "FDV",
                value: displaySafeValue(
                  safeNumber(geckoData?.attributes?.fdv_usd),
                  formatTokenPrice,
                ),
              },
              {
                label: "Liquidity",
                value: displaySafeValue(
                  safeNumber(geckoData?.attributes?.reserve_in_usd),
                  formatTokenPrice,
                ),
              },
              {
                label: "Volume 24h",
                value: displaySafeValue(
                  safeNumber(geckoData?.attributes?.volume_usd?.h24),
                  formatTokenPrice,
                ),
              },
              {
                ...
                value: displaySafeValue(
                  safeNumber(
                    geckoData?.attributes?.price_change_percentage?.h24,
                  ),
                  formatPercentage,
                ),
              },
                  ...
                  [
                    {
                      ...
                      sub:
                        mode === "FULL"
                          ? displaySafeValue(buyVolume, formatTokenPrice)
                          : "-",
                    },
                    {
                      label: "Sells",
                      ...
                      sub:
                        mode === "FULL"
                          ? displaySafeValue(sellVolume, formatTokenPrice)
                          : "-",
                    },
                    {
                      label: "Buyers",
                ...
                <div className="space-y-1 text-xs">
                  ...
                  {mode === "FULL" ? (
                    <div className="text-gray-400">
                      ...
                      <span
                        className={
                          netVolume! >= 0 ? "text-green-500" : "text-red-500"
                        }
                      >
                    ...
                    <div className="text-xs text-gray-500">
                      Net volume unavailable
                    </div>
                  )}
                </div>
              ) : (
                <p className="text-xs text-gray-500">
                  No trading activity in the last 24h
                </p>
              )}
            </section>
          ) : null}
