        ...
        try {
          ...
          const nextCursor =
            paginatedTokens.length > 0
              ? paginatedTokens[paginatedTokens.length - 1].tokenMint
              : undefined;
  ...
  return tokens
    .filter((token) => {
      switch (activeTab) {
        case "trending":
          return (
            (token.status === "PRE_GRAD" || token.status === "MIGRATED") &&
            !!token.geckoData?.data?.attributes
          );
        ...
        case "top_gainers":
          return (
            !!token.geckoData?.data?.attributes &&
            Number(
              token.geckoData?.data?.attributes.price_change_percentage.h24 ??
                0,
            ) > 0
          );
    ...
    .sort((a, b) => {
      switch (activeTab) {
        ...
        case "new":
          return (
            new Date(
              b.geckoData?.data?.attributes.pool_created_at ?? 0,
            ).getTime() -
            new Date(
              a.geckoData?.data?.attributes.pool_created_at ?? 0,
            ).getTime()
          );
        ...
        case "top_gainers":
          return (
            Number(
              b.geckoData?.data?.attributes.price_change_percentage.h24 ?? 0,
            ) -
            Number(
              a.geckoData?.data?.attributes.price_change_percentage.h24 ?? 0,
            )
          );
        ...
        case "top_bags":
          return (
            Number(b.geckoData?.data?.attributes.fdv_usd ?? 0) -
            Number(a.geckoData?.data?.attributes.fdv_usd ?? 0)
          );
