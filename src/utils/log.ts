...
function createBaseLogger(context: Partial<LogInput> = {}) {
  ...
  // Levels
  baseLog.info = (input: LogInput) =>
    baseLog({ ...input, tag: input.tag ?? "info" });
  baseLog.error = (input: LogInput) =>
    baseLog({ ...input, tag: input.tag ?? "error" });
  baseLog.warn = (input: LogInput) =>
    baseLog({ ...input, tag: input.tag ?? "warn" });
  baseLog.debug = (input: LogInput) =>
    baseLog({ ...input, tag: input.tag ?? "debug" });
  ...
  baseLog.child = (childContext: Partial<LogInput>) =>
    createBaseLogger({ ...context, ...childContext });
