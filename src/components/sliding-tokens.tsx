...
function Token({ value }: { value?: MergedBagsTokenWithPool }) {
  ...
  const percentChange = parseFloat(
    value?.geckoData?.data?.attributes.price_change_percentage.h24 ?? "0",
  );
    ...
    >
      <article className="flex items-center gap-2 text-xs">
        ...
        <span className="text-accent text-sm">
          {formatPriceToUSD(
            parseFloat(
              value?.geckoData?.data?.attributes.base_token_price_usd ?? "0",
            ),
          )}
        </span>
        <span
          className={cn(percentChange >= 0 ? "text-green-500" : "text-red-500")}
        >
