...
interface TokenDetailsModalProps {
    onClose: () => void;
    tokenMint: MergedBagsTokenWithPool["tokenMint"];
}

export function TokenDetailsModal({ tokenMint, onClose }: TokenDetailsModalProps) {
    const { data, isLoading } = useQuery(getTokenQueryOptions({ mint: tokenMint }));
    ...
    return (
        <Dialog open onOpenChange={(open) => !open && onClose()}>
            <DialogContent className="h-[calc(100dvh-4rem)] max-w-[calc(100dvw-2rem)] sm:max-w-[calc(100dvw-2rem)] lg:h-auto lg:max-w-5xl">
                {!token || isLoading ? (
                    <section className="flex h-[calc(86.5dvh)] items-center justify-center">
